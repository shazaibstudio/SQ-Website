...
';
  ...
  function showServices(worldId) {
    var world = KB.worlds[worldId];
    var lines = world.services.map(function (s) { return '• ' + s.name + ' — ' + s.hint; }).join('\n');
    bump(5);
    showTyping(function () {
      addBotMessage(
        world.label + ' services:\n' + lines,
        [
          { label: 'What does that cost?', action: 'pricing', world: worldId },
          { label: 'Show me proof', action: 'projects', world: worldId },
  ...
  function showPricing(worldId) {
    var world = KB.worlds[worldId];
    var lines = world.pricing.map(function (p) { return '• ' + p.tier + ' — ' + p.price + ' (' + p.note + ')'; }).join('\n');
    bump(15);
    State.budgetSignal = true;
    showTyping(function () {
      addBotMessage(
        world.label + ' pricing:\n' + lines + '\n\nActual cost depends on scope — happy to get you an exact number.',
